...
import { Box, Dialog, DialogTitle, DialogContent, DialogActions, Button, Alert, CircularProgress } from '@mui/material';
...
class BlocklyEditor extends React.Component<BlocklyEditorProps, BlocklyEditorState> {
  ...
  componentDidUpdate(prevProps: BlocklyEditorProps) {
    ...
    if (!this.state.isWorkspaceInitialized && this.blocklyDiv && this.props.toolbox) {
    ...
    if (this.state.isWorkspaceInitialized && this.state.workspace) {
      ...
      if (this.props.value !== prevProps.value && prevProps.value !== undefined) {
  ...
  private registerBlocks() {
    ...
    console.log('Starting block registration, total blocks:', this.props.toolbox.blocks.length);
          ...
          window.Blockly.Blocks[blockDef.type] = {
            init: function() {
              ...
              try {
                ...
                console.error(`Failed to initialize block ${blockDef.type}:`, error);
                throw error;
              }
            }
          };
    ...
    console.log('Block registration complete. Registered blocks:', Object.keys(window.Blockly.Blocks));
  }
  ...
  private updateToolbox() {
    ...
    try {
      console.log('Current blocks in registry:', Object.keys(window.Blockly.Blocks));
      ...
      // Update toolbox
      console.log('Updating workspace toolbox with new configuration:', this.props.toolbox);
      try {
        ...
        throw new Error(`Toolbox update failed: ${updateError instanceof Error ? updateError.message : 'Unknown error'}`);
      }
  ...
  private loadWorkspaceState() {
    ...
    try {
      ...
      try {
        window.Blockly.serialization.workspaces.load(state.blocks, this.state.workspace);
        console.log('Successfully loaded blocks');
      } catch (blockError) {
        console.error('Failed to load blocks:', blockError);
        throw new Error(`Block loading failed: ${blockError instanceof Error ? blockError.message : 'Unknown error'}`);
      }
        ...
        state.variables.forEach(variable => {
          ...
          if (!this.state.workspace.getVariableById(variable.id)) {
            try {
              ...
              console.error(`Failed to create variable ${variable.name}:`, varError);
              throw new Error(`Variable creation failed for ${variable.name}: ${varError instanceof Error ? varError.message : 'Unknown error'}`);
            }
          } else {
